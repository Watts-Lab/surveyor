...
router.get("/sa/:alias/", csrfProtection, async (req, res: Response) => {
  ...
  if (
    ...
    validations_first == false
  ) {
    res.redirect("/s")
  } else {
    // if validation_first is undefined it will default as first
  ...
  try {
    ...
    if (parsed.sent && parsed.WorkerId && parsed.url) {
      ...
      if (await isSurveyCompleted(parsed)) {
        return res
          .status(400)
          .send("This link has already been used for submission")
      }
    }
    ...
    return res
      .status(400)
      .send("Wrong encryption. No URL is found. Please email researcher.")
  }
})
...
router.get("/thanks", exists_token, async (req, res) => {
  ...
  if (admin == true) {
    let response = await Db_Wrapper.find(
      { session: req.session.id },
      "responses"
    )
    response = response[0]
...
router.post("/survey", csrfProtection, exists_token, async (req, res) => {
  ...
  if (
    !req.body["check"] ||
    Number(req.body["curr_page"]) > Number(req.body["final"])
  ) {
...
router.get("/results", verify_admin_token, async (req, res) => {
  await Db_Wrapper.find({}, "responses").then((all_responses) => {
