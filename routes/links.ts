...
router.post(`/link/survey`, verify_api_token, async (req, res) => {
  ...
  const is_alias: boolean =
    (await Db_Wrapper.find({ alias }, "survey_links")).length != 0
  while (is_alias) {
    // only unique aliases allowed
...
router.post(`/link/${env_config.RANDOM}`, async (req, res) => {
  ...
  await Db_Wrapper.update(
    { alias, active: true },
    { $set: { active: false, end: Date().toString() } },
    {},
    "links"
  )
  ...
  await Db_Wrapper.insert(
    { alias, url, hits: 0, active: true, start: Date().toString(), end: null },
    "links"
  )
...
router.get("/r/:alias", async (req, res) => {
  try {
    const active = true
    const body = await Db_Wrapper.find(
      { alias: req.params.alias, active },
      "links"
    )
